...
| `src/db.rs` | SQLite schema、批次/样本/审计/集成任务持久化、集成任务请求/回执 AES-256-GCM 加密；文件库已接入 SQLx SQLite pool，审计日志 total/list/chain、审计 CSV 导出读取、批次/产物结果 history、AI 推荐输入/缓存、实时曲线/v1 history/批次报告样本读取、AINAS/MQTT 集成任务查询/创建/更新已开始走 SQLx，schema migration 和部分兼容路径仍在迁移中 |
