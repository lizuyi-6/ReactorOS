...
| `src/db.rs` | SQLite schema、批次/样本/审计/集成任务持久化、集成任务请求/回执 AES-256-GCM 加密；文件库已接入 SQLx SQLite pool，审计日志 total/list/chain、审计 CSV 导出读取以及批次/产物结果 Recent history 读取已开始走 SQLx，主体写入和多数业务读写仍在迁移中 |
...
DB Recent/History 查询约定：实时样本、报警、批次、产物结果和审计事件类 Recent 接口先用 `ORDER BY id DESC LIMIT N` 限定“最新窗口”，再在外层按 `id ASC` 返回给 HMI/报告使用，保证用户看到的是窗口内从旧到新的时间线。PRD SQLx 技术栈迁移采用分段方式推进：当前文件数据库通过 SQLx SQLite pool 支撑 `/api/audit/logs` 的审计 total/list/chain 查询、`/api/audit/export.csv` 的导出读取，以及 `live`、demo context、批次列表、批次 CSV/XLSX 导出和 AI 实验计划中的 Recent 批次/产物结果读取；内存测试库和多数持久化路径继续使用 `rusqlite` 作为兼容层，后续逐步扩大 SQLx 覆盖面。
