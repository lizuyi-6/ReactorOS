...
| RBAC / 审计 | 本地 bearer session、角色权限、审计哈希链、CSV 导出 |
| AINAS 任务下发 | `POST/GET /api/integrations/ainas/tasks`，支持 `set_targets`、`start_process`、`stop_process`，任务持久化 |
| MQTT 基础桥 | `config/integration.toml`，`rumqttc` MQTT 3.1.1 client，默认 TLS/8883，订阅 task topic，发布 receipt topic，按 `alert_interval_s` 发布 retained 报警快照，payload 复用 AINAS 安全执行路径 |
| Modbus RTU/TCP 调试映射 | `/api/modbus/registers` 暴露 8 个读寄存器、7 个写寄存器、coils、discrete inputs；Modbus TCP PDU 支持 `01/02/03/06`；写目标走安全夹紧和审计 |
...
| 独立 safety guard | 新增 `reactor-safety-guard` 子进程和 stdin/stdout JSON 协议；`reactor-edge-daemon --safety-guard` 可委托自动控制安全判定，`xingshu safety check` 可本地验收夹紧结果 |
| CLI 上位机入口 | `xingshu` CLI 覆盖启动、状态、配置、数据、控制、AI、审计、Modbus 调试 |
| 数据导出 | 批次 CSV/XLSX、单批次 Markdown 实验报告、审计 CSV |
...
| MQTT | 配置、状态摘要、rumqttc client、任务执行、receipt 逻辑、alert topic 报警快照、CA/客户端证书配置字段已实现并测试 | 外部 broker 联调、断线重连验收、MQTT.fx 证书链验收 |
| 本地 AI | 本地传统优化器、AI 建议、云端 provider、CLI 显示 LoRA 缺口 | Qwen3.5-2B + LoRA 推理、训练、GGUF 转换、RK 端延迟验证 |
...
```powershell
$env:CARGO_TARGET_DIR='target-tls-check'
cargo fmt --check
...
- `api_tests`：42 passed，含配置摘要 AES 状态断言和 Modbus TCP TLS 回归。
- `cli_tests`：10 passed，新增 safety guard、AI SOP、perf help、数据样本入口等 CLI 覆盖。
- `db_tests`：6 passed，新增 AES-256-GCM 加密落盘和旧明文兼容读取测试。
- `control_tests`：4 passed，新增 safety guard 请求与进程内安全判定一致性测试。
