...
```powershell
Copy-Item data\reactor.sqlite3 backup\reactor-$(Get-Date -Format yyyyMMdd-HHmmss).sqlite3
...
| `xingshu ai train` 失败 | 本地 LoRA 训练资产缺失 | 这是当前预期缺口，需补 Qwen/GGUF/LoRA/训练脚本/RK 报告 |
...
当前维护手册足以支撑本地联调和验收准备。正式生产维护仍需补现场备份系统、密钥托管、证书链、watchdog/权限隔离、安全扫描和升级回滚演练证据。
