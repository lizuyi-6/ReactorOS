...
| --- | --- | --- | --- | --- | --- |
| D1 | 前端采用 Vue 3.4+、Vite、Element Plus、ECharts、Pinia | `frontend/` 已接入 Vue 3、Vite、Element Plus、ECharts、Pinia、Vue Router，包含 PRD 七大页面、Pinia 后端数据 store、ECharts 实时曲线和持久化中英切换；`npm run frontend:build` 与七路由中英 DOM/截图验证已通过；生产服务仍托管 `static/index.html` | 迁移中 | PRD 前端栈已开始落地，并具备可验证的七页面迁移版；控制写入、审计交互、Modbus 写入、生产替换和完整 parity 尚未完成 | 继续把 `static/index.html` 的控制表单、审计导出、Modbus 写入和剩余交互迁入 Vue；通过后再把 daemon 静态资源切到 `frontend/dist/index.html` |
...
PRD v2.2 指定 Vue 3.4+、Vite、Element Plus、ECharts 和 Pinia。`codex/prd-tech-stack-migration` 分支已经把 `frontend/` 从占位 TypeScript 页面推进为真实 Vue 应用：`App.vue` 承载工业 HMI shell，`router.ts` 映射 PRD 七大页面，`stores/plant.ts` 集中访问 `/health`、`/api/config/summary`、`/api/audit/logs`、`/api/modbus/registers` 和 `/api/recommendations/latest`，`MonitorView.vue` 使用 ECharts 绘制实时曲线，七个 Vue 页面已接入 Pinia 持久化中英切换。当前已用 `npm run frontend:build` 验证单文件构建，并用 Playwright/Chromium 对 `/#/monitor`、`/#/control`、`/#/ai`、`/#/history`、`/#/audit`、`/#/modbus`、`/#/settings` 做中文/英文 DOM 字块检查，截图证据位于 `output/playwright/vue-i18n-monitor-zh.png`、`output/playwright/vue-i18n-monitor-en.png`、`output/playwright/vue-i18n-modbus-zh.png` 和 `output/playwright/vue-i18n-modbus-en.png`。

当前生产 HMI 仍由 `static/index.html` 单文件原生实现提供，已覆盖实时监控、参数/工艺控制、AI、历史批次、物料/产品结果、报警、审计、Modbus、系统配置、中英切换和本地视觉验证。对外应表述为“PRD 前端技术栈迁移已进入可构建、可视觉验证的七页面 Vue 版本；生产 HMI 替换和功能 parity 验收待完成”，不能表述为“已按 PRD 前端栈完成生产交付”。
...
| P1 | 前端组件化迁移 | Vue/Element Plus/ECharts/Pinia 已有七页面迁移版和中英切换视觉证据；继续补齐功能 parity、写操作交互和生产静态资源切换 |
