...
| --- | --- | --- | --- | --- | --- |
| D1 | 前端采用 Vue 3.4+、Vite、Element Plus、ECharts、Pinia | `frontend/` 已接入 Vue 3、Vite、Element Plus、ECharts、Pinia、Vue Router，包含 PRD 七大页面、Pinia 后端数据 store、ECharts 实时曲线、持久化中英切换、控制目标写入、自动控制、人工锁定、急停/复位、审计链指标/事件过滤/分页/CSV 导出、Modbus 寄存器读回和 admin-only 调试写入；`npm run frontend:build`、七路由中英 DOM/截图验证、控制写入、审计导出和 Modbus 写入浏览器验证已通过；生产服务仍托管 `static/index.html` | 迁移中 | PRD 前端栈已开始落地，并具备可验证的七页面迁移版；工艺管理、生产替换和完整 parity 尚未完成 | 继续把 `static/index.html` 的工艺管理和剩余交互迁入 Vue；通过后再把 daemon 静态资源切到 `frontend/dist/index.html` |
