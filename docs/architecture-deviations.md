...
| D2 | 后端数据库采用 SQLx ORM | 文件数据库已接入 SQLx SQLite pool，审计日志 total/list/chain/export、批次/产物结果 history、AI 推荐输入/缓存、实时曲线/v1 history/批次报告样本、AINAS/MQTT 集成任务查询/创建/更新已由真实 API/MQTT 路径调用 SQLx；schema/migration 和部分保留兼容路径仍使用 `rusqlite` | 迁移中 | 已开始对齐 PRD 技术栈，但当前仍是混合数据库层；尚未获得全面 SQLx migration 封装和更广泛编译期 SQL 约束 | 继续把 schema migration 迁到 SQLx，随后收缩 `rusqlite` 到兼容/诊断工具路径 |
...
原因是本地 LoRA 和自进化仍未真实交付，SQLx 已覆盖主要运行时读路径和 AINAS/MQTT 集成任务创建/更新，但 schema migration 和部分兼容路径仍未完全迁移，Modbus TCP server 尚未迁到 `tokio-modbus`，Vue 前端栈也仍处于迁移壳和生产替换前状态。
