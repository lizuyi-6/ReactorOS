...
原因是本地 LoRA 和自进化仍未真实交付，SQLx 已覆盖主要运行时读路径、实时/外部样本写入、AI 推荐/产物结果写入和 AINAS/MQTT 集成任务创建/更新，但 schema migration 和部分兼容路径仍未完全迁移，Modbus TCP server 尚未迁到 `tokio-modbus`，Vue 前端栈也仍处于迁移壳和生产替换前状态。
