...
当前上位机已经达到“本地 PoC/联调验收”状态：Web HMI、七大页面主体、REST API、CLI、RBAC、审计链、批次数据、导出、AINAS REST、基础 MQTT、Modbus RTU/TCP 调试映射、安全限幅、HTTP/HTTPS、本地 AES 加密、独立 safety guard、LoRA 数据集导出、训练命令编排、manifest 归档和显式候选 adapter 晋级/备份边界均已有实现或本地测试证据。

不能宣称已完全满足 PRD 的内容主要集中在四类：

1. 本地 Qwen3.5-2B + LoRA 真实模型推理、自进化、GGUF 转换、生产训练脚本和 RK 端延迟验收。
...
| --- | --- | --- | --- | --- |
| 实时监控、参数配置、AI 智能决策、历史数据、审计日志、Modbus 调试、系统配置七大页面完整开发 | 本地通过 | `frontend/dist/index.html` 默认 Vue 生产构建；`static/index.html` legacy fallback；`scripts/verify-vue-release-assets.mjs`；Monitor 页已通过正式样本入口触发温度/压力越限报警并完成中英截图验证；AI 页已补 master-control 结构化复核和 SOP 结构化草案；`scripts/verify-vue-browser-matrix.mjs` 严格模式已覆盖 bundled Chromium、系统 Chrome、系统 Microsoft Edge、Firefox 和 WebKit 桌面七路由 × 中英，70 个页面/语言组合全通过，0 skipped，0 console error；`docs/upper_computer_development_doc.md`；`docs/upper_computer_test_report.md`；本地 `http://127.0.0.1:8000/health` 正常 | 需要正式用户验收记录、release/RK 运行证据、macOS Safari 和移动端真机验收记录 | 上位机、验收环境 |
| 中英切换与所有主要字块切换 | 本地通过 | `output/playwright/vue-i18n-verification.json`；`output/playwright/vue-parity-verification.json`；`output/playwright/vue-mobile-verification.json`；`output/playwright/vue-browser-matrix-verification.json`；`docs/upper_computer_test_report.md`；`docs/upper_computer_visual_evidence_index.md`；`output/playwright/vue-parity-ai-zh.png`；`output/playwright/vue-parity-ai-en.png` | 仍需 macOS Safari、iOS/Android 真机和正式用户验收记录 | 上位机 / 验收环境 |
| 工艺探索模块落地 | 部分完成 | 流程/批次生命周期、AI 建议、历史批次、曲线、报告、只读实验 SOP 草案、LoRA 训练数据集导出、manifest 和候选 adapter 晋级/备份边界已实现 | 本地 LoRA 真实模型深度参与探索、真实模型自主生成/评审闭环尚未完成 | 上位机 + 算法 |
| 手动/AI 自动双模式切换 | 本地通过 | `/api/control/auto`、`/api/control/targets`、自动控制安全门、HMI Control/AI 页面；AI 页已结构化展示 dry-run/execute 的决策、动作、安全门控和推荐目标 | 需要真实硬件执行闭环验证，证明 AI/自动目标能安全下发到 STM32 | 硬件 |
...
| 数据曲线渲染、数据存储与导出 | 本地通过 | SQLite 批次/样本；History 批次搜索、状态筛选、产物比例筛选、产物结果录入/联动、产率/产物比例/目标参数展示；CSV/XLSX/Markdown 报告；`output/playwright/vue-parity-verification.json` 和 `output/playwright/vue-history-xlsx-export-verification.json` 已覆盖 History CSV/XLSX 下载点击；`docs/upper_computer_test_report.md` | 需要真实实验数据持续采集后的完整报告样本 | 硬件/实验 |
...
| CLI 控制台 | 本地通过 | `xingshu` 覆盖 status/config/data/control/ai/audit/modbus/safety/perf，`xingshu ai plan` 可查看 SOP 草案，`xingshu ai train --export-only` 可从真实 SQLite 导出监督训练 JSONL，`xingshu ai train --manifest ... --promote --min-eval-score ...` 可按训练输出指标显式晋级候选 adapter 并保留备份，`xingshu perf smoke` 可输出本地性能冒烟报告 | 仍需真实 Qwen/GGUF/LoRA 资产、生产训练脚本和 RK 验收报告 | 算法 |
...
| 输出上位机开发文档、用户手册、测试报告 | 本地完成 | `docs/upper_computer_development_doc.md`、`docs/upper_computer_user_manual.md`、`docs/upper_computer_test_report.md`、`docs/upper_computer_delivery_readiness_index.md`、`docs/upper_computer_cli_reference.md`、`docs/upper_computer_maintenance_manual.md`、`docs/upper_computer_api_acceptance_manual.md`、`docs/upper_computer_modbus_register_map.md`、`docs/upper_computer_rk_deployment_acceptance_guide.md`、`docs/upper_computer_training_deck.md`、`docs/upper_computer_training_deck.pptx`、`docs/upper_computer_training_video_storyboard.md`、`outputs/manual-20260607-training/video/upper_computer_training_video_draft.mp4`、`docs/upper_computer_user_acceptance_script.md`、`docs/upper_computer_training_attendance_and_issues.md`、`docs/upper_computer_field_delivery_execution_pack.md` | 需要最终版随真实联调结果更新；现场最终版 PPTX、真实操作录屏 MP4 和真实签字仍待补 | 全团队 |
...
| AI 智能决策 | 本地 LoRA 参数建议 | 部分完成 | `/api/config/summary.local_ai.ready_for_lora_inference=false` 且 `ready_for_prd_lora=false`；`xingshu ai train --export-only` 可导出 `xingshu.local_ai.lora_dataset.v1` JSONL；配置训练脚本后可编排本地训练调用、写 `xingshu.local_ai.training_manifest.v1`，并在显式 `--promote` 时按评估分数晋级候选 adapter | Qwen3.5-2B/GGUF、生产 LoRA adapter、llama.cpp 推理接口、生产训练脚本、RK 延迟报告 |
...
| AI 智能决策 | 本地模型自动增量微调/自进化 | 部分完成 | `docs/local_ai_adapter_status_addendum.md` 明确为 readiness boundary；CLI 已具备训练数据集导出、训练入口编排、manifest、显式晋级和备份回滚路径 | 生产 PEFT/LoRA 训练脚本、最终数据集契约评审、自动触发、生产评估/审批流、RK 验收 |
| AI 智能决策 | AI 实验方案/SOP 自动生成 | 部分完成 | `GET /api/ai/experiment-plan` 和 `xingshu ai plan` 可基于批次推荐与安全边界生成只读三段式 SOP 草案；HMI AI 页已按摘要、步骤、验收指标、安全说明、模型边界和下一步结构化展示，并对已知固定文案做中英切换 | 仍缺云端/本地 LoRA 模型自主生成完整实验方案、人工审核流和真实执行闭环 |
| 安全控制 | 独立安全过滤器、非法参数拦截 | 本地通过 / 发布路径已补 | `reactor-safety-guard`、`xingshu safety check`、`control_tests`、`deploy/reactor-edge.service` 默认 `--safety-guard`、`scripts/verify-production-safety-guard.mjs` | 现场 watchdog、低权限账号确认和故障注入演练 |
...
| 数据审计与管理 | 历史查询、导出、可视化 | 本地通过 | HMI History 批次搜索、状态筛选、产物比例筛选、产物结果录入/联动、产率/产物比例/目标参数展示、CSV/XLSX、曲线；History CSV/XLSX 下载点击已纳入 Playwright parity gate 和独立 History XLSX gate | 真实数据样本验收 |
...
| 系统交互 | 完整 CLI | 本地通过 / AI 真实资产除外 | `xingshu` 命令集，含 LoRA 数据集导出、训练入口编排、manifest 和显式候选 adapter 晋级/备份 | 真实模型资产、生产训练脚本、推理接口和 RK 验收后补最终 AI 训练/推理证据 |
...
1. 本地 Qwen3.5-2B + LoRA：上位机已具备训练数据集导出、训练命令编排、manifest 和显式候选 adapter 晋级/备份；仍缺模型权重、生产 LoRA adapter、生产 PEFT 训练脚本、GGUF 转换脚本、RK 延迟报告和真实推理 API。
2. AI 自进化：已有训练数据集生成、训练产物归档、显式晋级和备份回滚边界；仍缺自动触发、生产评估/审批流、模型效果验证和“随实验准确率提升”的验证。
...
7. 生产安全：密钥生命周期和敏感字段清单已文档化，safety guard 发布和默认启动路径已补；仍缺生产密钥托管/轮换演练、正式渗透/漏洞扫描、现场 watchdog/低权限账号/故障注入演练。
...
9. 移动端/多浏览器：已有响应式实现、Chromium 桌面/手机/平板自动化截图、系统 Chrome、系统 Edge、Firefox、WebKit 桌面自动化截图和 Playwright 浏览器矩阵脚本；仍缺 macOS Safari、iOS/Android 真机和客户签字的完整验收记录。
10. 最终交付包：开发文档、用户手册、测试报告、API 手册、CLI 参考、维护手册、Modbus 映射手册、RK 部署验收指南、交付就绪索引、培训课件源稿、16 页可编辑 PPTX 草稿、video storyboard、静音 MP4 课件轮播草稿、用户验收操作脚本和培训签到问题模板已有初版；现场最终版 PPTX、真实操作录屏 MP4 视频和签字记录仍待补，全部文档需随真实硬件、外部接口和算法验收结果更新为最终版。
...
PRD 第十章交付物清单和当前证据对照见 `docs/upper_computer_delivery_readiness_index.md`。面向李祖祎汇报的短版缺口摘要见 `docs/upper_computer_current_gap_summary_for_lizuyi.md`。培训 PPT/视频的制作计划见 `docs/upper_computer_training_material_plan.md`；培训课件源稿见 `docs/upper_computer_training_deck.md`；PPTX 草稿见 `docs/upper_computer_training_deck.pptx`；视频 storyboard 见 `docs/upper_computer_training_video_storyboard.md`；静音 MP4 草稿见 `outputs/manual-20260607-training/video/upper_computer_training_video_draft.mp4`；用户验收操作脚本见 `docs/upper_computer_user_acceptance_script.md`；培训签到和问题闭环模板见 `docs/upper_computer_training_attendance_and_issues.md`；现场交付执行包说明见 `docs/upper_computer_field_delivery_execution_pack.md`。

PRD 指定技术栈与当前实现之间的偏离、影响和补偿措施见 `docs/architecture-deviations.md`。该文档同时修正一个容易误读的点：`reactor-safety-guard` 不是空壳，它会调用共享安全判断；生产发布和默认启动路径已补，真正剩余缺口是现场 watchdog、低权限账号确认和故障演练。
...
| P0 | 本地 LoRA 真实资产补齐 | 上位机训练数据集导出、训练入口编排、manifest 和显式候选 adapter 晋级/备份已补；继续补 Qwen/GGUF/LoRA/生产训练脚本/RK 报告，随后接入 daemon 推理 API 与自动触发/审批流 |
| P1 | 性能与安全报告 | 本地 API/安全计算冒烟、Windows debug 资源快照、密钥生命周期/敏感字段清单已补；继续补采集/执行控制延迟、release/RK 稳态 CPU/内存、生产证书链/密钥轮换演练、安全扫描报告 |
| P1 | 用户验收与部署 | RK 部署验收指南、培训材料计划、培训课件源稿、16 页可编辑 PPTX 草稿、video storyboard、静音 MP4 草稿、用户验收操作脚本和培训签到问题模板已输出；继续补移动端、多浏览器、全新设备部署计时、现场最终版 PPTX、真实操作录屏 MP4 和用户验收签字记录 |
...
- `GET /api/config/summary` 显示：
  ...
  - `local_ai.ready_for_base_inference = false`，`local_ai.ready_for_lora_inference = false`，`local_ai.ready_for_training = false`，`local_ai.ready_for_prd_lora = false`。
