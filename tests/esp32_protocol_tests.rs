...
fn parses_valid_esp32_rx_frame_with_checksum() {
    let body = "RX|v=1|seq=123|ms=456789|temp=175.4|pressure=0.21|stir_speed=450|shake_speed=30|flow_rate=2.5|product_concentration=62.4|ph=7.18";
...
fn rejects_esp32_frame_with_bad_checksum() {
    let frame = "RX|v=1|seq=123|ms=456789|temp=175.4|pressure=0.21|stir_speed=450|shake_speed=30|flow_rate=2.5|chk=00";
...
fn rejects_esp32_frame_missing_required_sensor() {
    let body = "RX|v=1|seq=123|ms=456789|temp=175.4|pressure=0.21|stir_speed=450|flow_rate=2.5";
    let frame = format!("{body}|chk={}", checksum_hex(body.as_bytes()));
    ...
    assert!(err.contains("missing field shake_speed"));
}

#[test]
fn parses_minimum_document_rx_frame_without_optional_lab_fields() {
    let body = "RX|v=1|seq=124|ms=456999|temp=176.1|pressure=0.22|stir_speed=455|shake_speed=31|flow_rate=2.6";
    let frame = format!("{body}|chk={}", checksum_hex(body.as_bytes()));
...
fn keeps_legacy_esp32_rx_aliases_for_existing_benches() {
    let body = "RX|v=1|seq=125|ms=457111|temp=175.4|pressure=0.21|rpm=450|shake=30|flow=2.5|conc=62.4|ph=7.18";
    let frame = format!("{body}|chk={}", checksum_hex(body.as_bytes()));
