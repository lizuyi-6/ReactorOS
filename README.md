...
运行在树莓派上的反应釜边缘上位机 PoC。系统通过 ESP32 串口桥或模拟设备采集反应釜数据，在树莓派本地完成安全限幅、批次记录、AI 推荐和 Web 上位机展示。
...
- 通信：ESP32 Serial 或模拟设备；配置中保留 Modbus RTU 映射。
- 部署：单二进制 + `systemd`，也支持 Docker Compose。

## 快速启动

默认配置使用模拟设备，不需要接 ESP32。
...
--enable-test-reset   启用 /api/test/reset，仅用于本地验收测试
```
...
设备通信配置。默认 `mode = "simulator"`，用于无硬件演示和测试。硬件模式使用 `config/device.esp32.toml`。
...
本地 E2E 使用的 `/api/test/reset` 只有在启动参数包含 `--enable-test-reset` 时可用，生产部署不要开启。
