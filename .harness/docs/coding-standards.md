...
**技术选型以"贴合项目 DNA"为先,不为开发效率妥协。** 这意味着:

- 工业控制 + 嵌入式 + 无网 kiosk 环境 → 不引外部运行时依赖 (CDN/字体/远程 API)
- 现场部署"打开就跑" → 不要打包器 / 框架 / 中间编译步骤
- 长期运行 + 单进程稳定 → 重量级依赖 (大型 SPA 框架、重 ML 推理) 一票否决
- 协议 / API 变更影响硬件 → 三个对端 (daemon / 固件 / 文档) 必须同步

**当"开发快"和"项目 DNA"冲突时,默认选项目 DNA,除非用户明确说这次破例。**
